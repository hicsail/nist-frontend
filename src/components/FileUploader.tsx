...
function FileUploader({ s3BucketName, loadFiles }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px', textAlign: 'center' }}>
      <input id="fileInput" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx" onChange={handleFileSelect} style={{ display: 'none' }} />
      <Typography variant="h6" gutterBottom>
        Select or drag and drop a file
      </Typography>
      <Paper sx={dropZoneStyle} onClick={handleDropZoneClick} onDragOver={handleDragOver} onDrop={handleDrop}>
