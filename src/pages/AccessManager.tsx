...
const AccessManager = () => {
  ...
  const filteredUserPermissions = userPermissions
    ? userPermissions.filter((userPermission: any) => userPermission.user.email.toLowerCase().includes(searchText.toLowerCase()))
    : [];
  ...
  return (
    <div>
      ...
      {organizationsWithAdminAccess.length === 0 ? (
        ...
        <>
          <InputLabel id="select-organization-label" style={{ marginTop: 40 }}>
          ...
          {currentOrganization && (
            <TextField id="outlined-basic" label="Search users by email" variant="outlined" fullWidth value={searchText} onChange={handleSearchChange} style={{ marginTop: 40 }} />
          )}
          {currentOrganization ? (
            <TableContainer style={{ marginTop: 10 }}>
              <Table>
                <EnhancedTableHead onRequestSort={handleRequestSort} columns={columns} sortableIds={['email']} order={order} orderBy={orderBy} />
                <TableBody>
                  {visibleUserPermissions.map(
                    (userPermission: any, index: number) =>
                      userPermission.user.id != userId && (
                        <TableRow key={userPermission._id}>
                          ...
                          <TableCell>
                            <Checkbox checked={userPermission.read} onChange={(event) => handlePermissionChange(index, 'read', event.target.checked)} />
                          </TableCell>
                          <TableCell>
                            <Checkbox checked={userPermission.write} onChange={(event) => handlePermissionChange(index, 'write', event.target.checked)} />
                          </TableCell>
                          <TableCell>
                            <Checkbox checked={userPermission.delete} onChange={(event) => handlePermissionChange(index, 'delete', event.target.checked)} />
                          </TableCell>
                          <TableCell>
